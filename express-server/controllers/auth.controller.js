...
exports.logOut = (req, res) => {
	res.clearCookie('mini-project', cookieConfig).end()
}
...
exports.authMiddleware = (req, res, next) => {
	const cookie = req.cookies['mini-project']
	...
	try {
		...
		console.log(err)
		return res.clearCookie('mini-project', cookieConfig).status(401).json({ message: err.message }).end()
	}
}
...
function createCookie(res, token) {
	res.cookie('mini-project', JSON.stringify({ token }), {
