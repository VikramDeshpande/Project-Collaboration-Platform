      ...
      <>
        ...
        <WithLabel label='Deadline (in days)'>
          <input
            type='number'
            value={form.deadline}
            onChange={(e) => dispatch({ type: 'deadline', value: parseInt(e.target.value, 10) })}
...
export type T = 'type' | 'summary' | 'descr' | 'assignee' | 'priority' | 'listId' | 'deadline';
...
const reducer = (state: State, { type, value }: A): State => {
  switch (type) {
    ...
    case 'deadline':
      return { ...state, deadline: value as number };
    default:
      return state;
  }
};
