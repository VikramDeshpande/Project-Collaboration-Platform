...
import { IssueMetaData } from '../issue/IssueModelHOC';
import { selectIssuesArray } from '../../api/endpoints/issues.endpoint';
...
const Issues = () => {
  ...
  return (
    <div className='mt-6 flex grow flex-col px-8 sm:px-10'>
      <h1 className='mb-4 text-xl font-semibold text-c-text'>Issues</h1>
      <Filter isEmpty={lists?.length === 0} {...{ projectId, setIsDragDisabled }} />

      {lists ? (
        <BoardRow {...{ lists, issues, isDragDisabled }} />
      ) : (
