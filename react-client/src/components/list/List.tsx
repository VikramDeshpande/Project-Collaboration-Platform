...
const List = (props: Props) => {
  ...
  const { members } = selectMembers(projectId) || { members: [] };
  const { authUser: u } = selectAuthUser() || { authUser: undefined };
  const isAdmin = members ? members.find(({ userId: uid }) => uid === u?.id)?.isAdmin : false;
